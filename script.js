...
function setupCloudModal() {
    ...
    if(syncBtn) {
        syncBtn.onclick = async () => {
            if (!confirm('Hành động này sẽ XÓA dữ liệu trên máy này và lấy bản mới nhất từ Cloud. Bạn có chắc chắn?')) return;
            showCloudStatus('⏳ Đang tải dữ liệu từ Cloud...', '#eef2ff', '#3730a3');
            await syncFromCloud(true); // Force overwrite
            showCloudStatus('✅ Đã tải thành công!', '#f0fdf4', '#166534');
        };
    }
    ...
    if(pushBtn) {
        pushBtn.onclick = async () => {
            if (!confirm('Hành động này sẽ ĐẨY dữ liệu của máy này lên Cloud, ghi đè toàn bộ dữ liệu hiện tại trên Cloud. Bạn có chắc chắn?')) return;
            showCloudStatus('⏳ Đang đẩy dữ liệu lên Cloud...', '#fffbeb', '#b45309');
            const success = await saveCloudData();
            if (success) {
                showCloudStatus('✅ Đã đẩy thành công!', '#f0fdf4', '#166534');
            } else {
                showCloudStatus('❌ Lỗi khi đẩy lên Cloud', '#fef2f2', '#991b1b');
            }
        };
    }
}
...
async function syncFromCloud(forceOverwrite = false) {
    const url = localStorage.getItem('finance_cloud_url');
    if (!url) {
    ...
    try {
        ...
        if (data.transactions && Array.isArray(data.transactions)) {
            // Force overwrite OR Cloud has more data
            if (forceOverwrite || data.transactions.length >= transactions.length) {
                transactions = data.transactions;
                updated = true;
            }
        }
        if (data.scheduledBills && Array.isArray(data.scheduledBills)) {
            if (forceOverwrite || data.scheduledBills.length >= scheduledBills.length) {
                scheduledBills = data.scheduledBills;
                updated = true;
            }
        }
        if (data.iceLogs && Array.isArray(data.iceLogs)) {
            if (forceOverwrite || data.iceLogs.length >= iceLogs.length) {
                iceLogs = data.iceLogs;
                updated = true;
            }
        }
        if (data.icePayments && Array.isArray(data.icePayments)) {
            if (forceOverwrite || data.icePayments.length >= icePayments.length) {
                icePayments = data.icePayments;
                updated = true;
            }
        }
        
        if (updated || forceOverwrite) {
...
window.showBreakdown = function(type) {
    ...
    // All-time totals so this matches the number shown on the dashboard card
    transactions.forEach(tx => {
    ...
    let html = `<div style="margin-bottom: 15px; text-align: center;">
                    <div style="font-size: 0.9rem; color: var(--text-muted);">Tổng cộng</div>
